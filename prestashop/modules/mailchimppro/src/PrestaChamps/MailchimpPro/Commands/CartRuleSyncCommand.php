...
class CartRuleSyncCommand extends BaseApiCommand
{
    ...
    protected function buildCartRules()
    {
        foreach ($this->cartRules as $cartRule) {
            $promoRuleFormatter = new PromoRuleFormatter($cartRule, $this->context);
            ...
            if ($this->method === self::SYNC_METHOD_POST) {
                if ($this->syncMode == self::SYNC_MODE_BATCH) {
                    ...
                    $this->commands[$cartRule->id]['pr'] = [
                        'route' => "ecommerce/stores/{$this->getShopId()}/promo-rules",
                        'data' => $promoRuleFormatter->format(),
                    ];
                    $this->commands[$cartRule->id]['pc'] = [
                        'route' => "ecommerce/stores/{$this->getShopId()}/promo-rules/{$cartRule->id}/promo-codes",
                ...
                if ($this->syncMode == self::SYNC_MODE_BATCH) {
                    ...
                    $this->commands[$cartRule->id]['pr'] = [
                        'route' => "ecommerce/stores/{$this->getShopId()}/promo-rules/{$cartRule->id}",
                        'data' => $promoRuleFormatter->format(),
                    ];
                    $this->commands[$cartRule->id]['pc'] = [
                        'route' => "ecommerce/stores/{$this->getShopId()}/promo-rules/{$cartRule->id}/promo-codes/{$cartRule->id}",
                        'data' => $promoRuleFormatter->formatPromoCode(),
                    ];
                }
